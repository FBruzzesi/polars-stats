...
An invalid parameter *value* (e.g. `sigma <= 0`) raises a `ComputeError` at evaluation; it never silently nulls.
Scalar parameters are validated once (they route through a constant-parameter fast path), column parameters per row;
a bad scalar and a bad column row surface identically.

```python exec="yes" source="above" session="nulls-and-errors" result="python"
df = pl.DataFrame({"x": [0.5]})

try:
    df.with_columns(ps.Normal(mu=0.0, sigma=-1.0).pdf("x"))
...
| Invalid parameter *value*, scalar or column row (`sigma <= 0`, `max <= min`, `p` outside `[0, 1]`) | Rust evaluation | `ComputeError`, fails the whole evaluation, never silently nulls |
| Any input `null` on a row | per row | `null` on that row |
| `q` outside `[0, 1]` in `ppf` | per row | implementation-defined; `null` for most distributions, do not rely on it |
