...
def _assert_matches_across_contexts(
    ...
    compute the identical value; the caller relaxes it only for `compares_bit_exactly`.
    """
...
@given(data=st.data())
def test_moment_fast_path_matches_per_row_across_contexts(spec: DistSpec, moment: Moment, data: st.DataObject) -> None:
    ...
    slow = getattr(spec.build("column", params), moment)()
    # Every input is constant, so the moment is a scalar column.
    _assert_matches_across_contexts(frame, fast, slow, fast_height=1, exact=compares_bit_exactly(spec.name, moment))
...
def test_value_keyed_fast_path_matches_per_row_across_contexts(spec: DistSpec, data: st.DataObject) -> None:
    ...
    cases = (
        (density(scalar, x), density(per_row, x)),
        (log_density(scalar, x), log_density(per_row, x)),
