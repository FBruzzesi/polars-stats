...
def test_log_cdf_matches_scipy(
    mn: float,
    mx: float,
    ...
    xs = value_grid(mn, mx)
    got = pl.DataFrame({"x": xs}).select(r=Uniform(min=mn, max=mx).log_cdf(pl.col("x")))["r"].to_numpy()
    np.testing.assert_allclose(got, scipy_uniform(mn, mx).logcdf(xs), atol=1e-12, rtol=0)
