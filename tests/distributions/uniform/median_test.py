...
def test_median_matches_scipy(
    mn: float,
    mx: float,
    unit_frame: pl.DataFrame,
    scipy_uniform: Callable[[float, float], Any],
) -> None:
    got = unit_frame.select(r=Uniform(min=mn, max=mx).median()).item(0, "r")
    assert got == pytest.approx(scipy_uniform(mn, mx).median(), abs=1e-12)
