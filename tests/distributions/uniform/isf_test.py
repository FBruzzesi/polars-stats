...
def test_isf_matches_scipy(mn: float, mx: float, scipy_uniform: Callable[[float, float], Any]) -> None:
    got = pl.DataFrame({"q": _QUANTILES}).select(r=Uniform(min=mn, max=mx).isf(pl.col("q")))["r"].to_numpy()
    np.testing.assert_allclose(got, scipy_uniform(mn, mx).isf(_QUANTILES), atol=1e-12, rtol=0)
