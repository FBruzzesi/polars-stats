...
def test_mean_column_p() -> None:
    ...
    result = df.select(v=Bernoulli(p=pl.col("p")).mean())["v"]
    expected = pl.Series("v", probs, dtype=pl.Float64)
    assert_series_equal(result, expected)


def test_mean_propagates_null_in_p(p_with_null: pl.DataFrame) -> None:
    result = p_with_null.select(v=Bernoulli(p=pl.col("p")).mean())["v"]
    expected = pl.Series("v", [0.3, None, 0.8], dtype=pl.Float64)
    assert_series_equal(result, expected)
