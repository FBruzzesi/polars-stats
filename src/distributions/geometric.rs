...
use crate::rng::{
    ...
    samples_per_row, samples_u64_output, SampleKwargs, SampleScalarKwargs, SamplesKwargs,
    SamplesScalarKwargs,
};

fn build_dist(p: f64) -> PolarsResult<Geometric> {
    if !p.is_finite() {
        return Err(PolarsError::InvalidOperation(format!("p must be finite, got {p}").into()));
    }
    Geometric::new(p).map_err(|e| {
...
/// `InvalidOperation` if `p` is outside `(0, 1]` or non-finite. `null` propagates.
///
/// The closed-form Python methods derive from this so they report an invalid `p` consistently
/// with `geometric_sample`, instead of silently computing a negative probability.
#[polars_expr(output_type=Float64)]
fn geometric_p(inputs: &[Series]) -> PolarsResult<Series> {
    let p = inputs[0].cast(&DataType::Float64)?;

    validate_params_unary(p.f64()?, |p| {
        build_dist(p)?;
        Ok(p)
    })
}
...
/// structural rather than only sampled by `sample_test.py`. The statrs draw is `f64`; the plugin
/// returns the integer support as `u64`.
#[inline]
fn draw(dist: &Geometric, rng: &mut impl rand::Rng) -> u64 {
    let x: f64 = <Geometric as Distribution<f64>>::sample(dist, rng);
    x as u64
}
...
fn geometric_samples(inputs: &[Series], kwargs: SamplesKwargs) -> PolarsResult<Series> {
    ...
    let rows = binary_param_rows(p.f64()?, index.u64()?, build_dist);
